function [data, bound_x, bound_y, bound_z, dataunit] = readdcmfiles(path, name, varargin)
%% This file is modified from the original by Tomppa Pakarinen 5.5.2020 (Original author: Mark Geurts, mark.w.geurts@gmail.com Copyright (C) 2016 University of Wisconsin Board of Regents) 
% Modifications made for dimension vectors and patient positioning / dose
% rotations
...
if abs(max(widths) - min(widths))/mean(widths) > 0.01
    if exist('Event', 'file') == 2
            Event(['Vector positions differ by more than 1%, suggesting ', ...
                'variable dose grid spacing. This is not suppforted.'], ...
                'ERROR');
        else
...
if info.GridFrameOffsetVector(end) < info.GridFrameOffsetVector(1)
    disp('GridFrameOffsetVector descending');
...
% Otherwise, if GridFrameOffsetVector is ascending
    ...
    disp('GridFrameOffsetVector ascending');
    % Read in dose data, converting to double
    dose.data = flip(flip(rot90(double(squeeze(dicomread(info)))),2),3) * ...
        info.DoseGridScaling;
end
